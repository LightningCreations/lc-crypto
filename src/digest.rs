...
impl<D: Digest> Digest for Hmac<D> {
    ...
    fn do_final(&mut self, lblock: &[u8], output: &mut [u8]) {
        let mut tmp_out = Zeroizing::new(vec![0u8; D::OUTPUT_SIZE].into_boxed_slice());
...
mod test {
    ...
    #[test]
    fn sha1_test_really_longstr2() {
        let input = [0; 65];
        let mut out = [0; 20];
        let expected = [
            0xf0, 0xfa, 0x45, 0x90, 0x6b, 0xd0, 0xf4, 0xc3, 0x66, 0x8f, 0xcd, 0x0d, 0x8f, 0x68,
            0xd4, 0xb2, 0x98, 0xb3, 0x0e, 0x5b,
        ];
        super::digest(Sha1::new(), &input, &mut out);
        assert_eq!(out, expected);
    }
    ...
    #[test]
    fn sha1_test_really_longstr3() {
        let input = b"This is a really, really, really long string. Don't question the contents.";
        let mut out = [0; 20];
        let expected = [
            0x30, 0xb0, 0x77, 0xbf, 0x91, 0xb8, 0xeb, 0xe6, 0xc7, 0x0a, 0xc9, 0x59, 0xb9, 0x5d,
            0x99, 0x7d, 0x21, 0x56, 0x09, 0x0c,
        ];
        super::digest(Sha1::new(), input, &mut out);
        assert_eq!(out, expected);
    }
