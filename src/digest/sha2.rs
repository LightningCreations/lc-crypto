use alloc::format;
use zeroize::Zeroize;

use super::Digest;

fn do_sha32_block(block: &[u8], h: &mut [u32; 8]) {
    let k = [
        0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4,
        0xab1c5ed5, 0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe,
        0x9bdc06a7, 0xc19bf174, 0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f,
        0x4a7484aa, 0x5cb0a9dc, 0x76f988da, 0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7,
        0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967, 0x27b70a85, 0x2e1b2138, 0x4d2c6dfc,
        0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85, 0xa2bfe8a1, 0xa81a664b,
        0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070, 0x19a4c116,
        0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
        0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7,
        0xc67178f2,
    ];
    let block = bytemuck::cast_slice::<u8, [u8; 4]>(block);
    let mut w = [0u32; 64];
    for i in 0..16 {
        w[i] = u32::from_be_bytes(block[i]);
    }

    for i in 16..64 {
        let s0 = w[i - 15].rotate_right(7) ^ w[i - 15].rotate_right(18) ^ (w[i - 15] >> 3);
        let s1 = w[i - 2].rotate_right(17) ^ w[i - 2].rotate_right(19) ^ (w[i - 2] >> 10);
        w[i] = w[i - 16]
            .wrapping_add(s0)
            .wrapping_add(w[i - 7])
            .wrapping_add(s1);
    }

    let mut a = h[0];
    let mut b = h[1];
    let mut c = h[2];
    let mut d = h[3];
    let mut e = h[4];
    let mut f = h[5];
    let mut g = h[6];
    let mut l = h[7];

    for i in 0..64 {
        let s1 = e.rotate_right(6) ^ e.rotate_right(11) ^ e.rotate_right(25);
        let ch = (e & f) ^ ((!e) & g);
        let temp1 = l
            .wrapping_add(s1)
            .wrapping_add(ch)
            .wrapping_add(k[i])
            .wrapping_add(w[i]);
        let s0 = a.rotate_right(2) ^ a.rotate_right(13) ^ a.rotate_right(22);
        let maj = (a & b) ^ (a & c) ^ (b & c);
        let temp2 = s0.wrapping_add(maj);

        l = g;
        g = f;
        f = e;
        e = d.wrapping_add(temp1);
        d = c;
        c = b;
        b = a;
        a = temp1.wrapping_add(temp2);
    }

    h[0] = h[0].wrapping_add(a);
    h[1] = h[1].wrapping_add(b);
    h[2] = h[2].wrapping_add(c);
    h[3] = h[3].wrapping_add(d);
    h[4] = h[4].wrapping_add(e);
    h[5] = h[5].wrapping_add(f);
    h[6] = h[6].wrapping_add(g);
    h[7] = h[7].wrapping_add(l);
}

pub struct Sha32<const BITS: usize> {
    h: [u32; 8],
    size: u64,
}

impl Sha32<224> {
    pub const fn new() -> Self {
        Self {
            h: [
                0xc1059ed8, 0x367cd507, 0x3070dd17, 0xf70e5939, 0xffc00b31, 0x68581511, 0x64f98fa7,
                0xbefa4fa4,
            ],
            size: 0,
        }
    }
}

impl Sha32<256> {
    pub const fn new() -> Self {
        Self {
            h: [
                0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f, 0x9b05688c, 0x1f83d9ab,
                0x5be0cd19,
            ],
            size: 0,
        }
    }
}

impl Default for Sha32<224> {
    fn default() -> Self {
        Self::new()
    }
}

impl Default for Sha32<256> {
    fn default() -> Self {
        Self::new()
    }
}

impl<const BITS: usize> Digest for Sha32<BITS> {
    const BLOCK_SIZE: usize = 64;
    const OUTPUT_SIZE: usize = BITS / 8;

    fn init(&mut self) {
        // This would work so well with specialization
        self.size = 0;
        self.h = match BITS {
            224 => [
                0xc1059ed8, 0x367cd507, 0x3070dd17, 0xf70e5939, 0xffc00b31, 0x68581511, 0x64f98fa7,
                0xbefa4fa4,
            ],
            256 => [
                0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f, 0x9b05688c, 0x1f83d9ab,
                0x5be0cd19,
            ],
            _ => unreachable!(),
        }
    }

    fn update(&mut self, block: &[u8]) {
        self.size = block.len() as u64 * 8;
        do_sha32_block(block, &mut self.h)
    }

    fn do_final(&mut self, mut lblock: &[u8], out: &mut [u8]) {
        assert!(lblock.len() <= 64);
        if lblock.len() == 64 {
            self.update(lblock);
            lblock = &[];
        }
        let len = lblock.len();
        let mut bytes = [0u8; 64];
        bytes[..len].copy_from_slice(lblock);
        self.size += (lblock.len() as u64) * 8;
        let ml = self.size;
        bytes[len] = 0x80;
        if (64 - len) < 8 {
            self.update(&bytes);
            bytes = [0u8; 64];
        }
        let len = bytes.len() - 8;
        bytes[len..].copy_from_slice(&ml.to_be_bytes());
        self.update(&bytes);
        bytes.zeroize();
        let out = bytemuck::cast_slice_mut::<u8, [u8; 4]>(out);
        for i in 0..(BITS / 32) {
            out[i] = self.h[i].to_be_bytes();
        }
    }
}

pub type Sha224 = Sha32<224>;
pub type Sha256 = Sha32<256>;

fn do_sha64_block(block: &[u8], h: &mut [u64; 8]) {
    let k = [
        0x428a2f98d728ae22,
        0x7137449123ef65cd,
        0xb5c0fbcfec4d3b2f,
        0xe9b5dba58189dbbc,
        0x3956c25bf348b538,
        0x59f111f1b605d019,
        0x923f82a4af194f9b,
        0xab1c5ed5da6d8118,
        0xd807aa98a3030242,
        0x12835b0145706fbe,
        0x243185be4ee4b28c,
        0x550c7dc3d5ffb4e2,
        0x72be5d74f27b896f,
        0x80deb1fe3b1696b1,
        0x9bdc06a725c71235,
        0xc19bf174cf692694,
        0xe49b69c19ef14ad2,
        0xefbe4786384f25e3,
        0x0fc19dc68b8cd5b5,
        0x240ca1cc77ac9c65,
        0x2de92c6f592b0275,
        0x4a7484aa6ea6e483,
        0x5cb0a9dcbd41fbd4,
        0x76f988da831153b5,
        0x983e5152ee66dfab,
        0xa831c66d2db43210,
        0xb00327c898fb213f,
        0xbf597fc7beef0ee4,
        0xc6e00bf33da88fc2,
        0xd5a79147930aa725,
        0x06ca6351e003826f,
        0x142929670a0e6e70,
        0x27b70a8546d22ffc,
        0x2e1b21385c26c926,
        0x4d2c6dfc5ac42aed,
        0x53380d139d95b3df,
        0x650a73548baf63de,
        0x766a0abb3c77b2a8,
        0x81c2c92e47edaee6,
        0x92722c851482353b,
        0xa2bfe8a14cf10364,
        0xa81a664bbc423001,
        0xc24b8b70d0f89791,
        0xc76c51a30654be30,
        0xd192e819d6ef5218,
        0xd69906245565a910,
        0xf40e35855771202a,
        0x106aa07032bbd1b8,
        0x19a4c116b8d2d0c8,
        0x1e376c085141ab53,
        0x2748774cdf8eeb99,
        0x34b0bcb5e19b48a8,
        0x391c0cb3c5c95a63,
        0x4ed8aa4ae3418acb,
        0x5b9cca4f7763e373,
        0x682e6ff3d6b2b8a3,
        0x748f82ee5defb2fc,
        0x78a5636f43172f60,
        0x84c87814a1f0ab72,
        0x8cc702081a6439ec,
        0x90befffa23631e28,
        0xa4506cebde82bde9,
        0xbef9a3f7b2c67915,
        0xc67178f2e372532b,
        0xca273eceea26619c,
        0xd186b8c721c0c207,
        0xeada7dd6cde0eb1e,
        0xf57d4f7fee6ed178,
        0x06f067aa72176fba,
        0x0a637dc5a2c898a6,
        0x113f9804bef90dae,
        0x1b710b35131c471b,
        0x28db77f523047d84,
        0x32caab7b40c72493,
        0x3c9ebe0a15c9bebc,
        0x431d67c49c100d4c,
        0x4cc5d4becb3e42b6,
        0x597f299cfc657e2a,
        0x5fcb6fab3ad6faec,
        0x6c44198c4a475817,
    ];
    let block = bytemuck::cast_slice::<u8, [u8; 8]>(block);
    let mut w = [0u64; 80];
    for i in 0..16 {
        w[i] = u64::from_be_bytes(block[i]);
    }

    for i in 16..80 {
        let s0 = w[i - 15].rotate_right(1) ^ w[i - 15].rotate_right(8) ^ (w[i - 15] >> 7);
        let s1 = w[i - 2].rotate_right(19) ^ w[i - 2].rotate_right(61) ^ (w[i - 2] >> 6);
        w[i] = w[i - 16]
            .wrapping_add(s0)
            .wrapping_add(w[i - 7])
            .wrapping_add(s1);
    }

    let mut a = h[0];
    let mut b = h[1];
    let mut c = h[2];
    let mut d = h[3];
    let mut e = h[4];
    let mut f = h[5];
    let mut g = h[6];
    let mut l = h[7];

    for i in 0..80 {
        let s1 = e.rotate_right(14) ^ e.rotate_right(18) ^ e.rotate_right(41);
        let ch = (e & f) ^ ((!e) & g);
        let temp1 = l
            .wrapping_add(s1)
            .wrapping_add(ch)
            .wrapping_add(k[i])
            .wrapping_add(w[i]);
        let s0 = a.rotate_right(28) ^ a.rotate_right(34) ^ a.rotate_right(39);
        let maj = (a & b) ^ (a & c) ^ (b & c);
        let temp2 = s0.wrapping_add(maj);

        l = g;
        g = f;
        f = e;
        e = d.wrapping_add(temp1);
        d = c;
        c = b;
        b = a;
        a = temp1.wrapping_add(temp2);
    }

    h[0] = h[0].wrapping_add(a);
    h[1] = h[1].wrapping_add(b);
    h[2] = h[2].wrapping_add(c);
    h[3] = h[3].wrapping_add(d);
    h[4] = h[4].wrapping_add(e);
    h[5] = h[5].wrapping_add(f);
    h[6] = h[6].wrapping_add(g);
    h[7] = h[7].wrapping_add(l);
}

pub struct Sha64<const BITS: usize> {
    h: [u64; 8],
    size: u128,
}

impl Sha64<384> {
    pub const fn new() -> Self {
        Self {
            h: [
                0xcbbb9d5dc1059ed8,
                0x629a292a367cd507,
                0x9159015a3070dd17,
                0x152fecd8f70e5939,
                0x67332667ffc00b31,
                0x8eb44a8768581511,
                0xdb0c2e0d64f98fa7,
                0x47b5481dbefa4fa4,
            ],
            size: 0,
        }
    }
}

impl Sha64<512> {
    pub const fn new() -> Self {
        Self {
            h: [
                0x6a09e667f3bcc908,
                0xbb67ae8584caa73b,
                0x3c6ef372fe94f82b,
                0xa54ff53a5f1d36f1,
                0x510e527fade682d1,
                0x9b05688c2b3e6c1f,
                0x1f83d9abfb41bd6b,
                0x5be0cd19137e2179,
            ],
            size: 0,
        }
    }
}

impl Sha64<224> {
    pub const fn new() -> Self {
        Self {
            h: [
                0x8C3D37C819544DA2,
                0x73E1996689DCD4D6,
                0x1DFAB7AE32FF9C82,
                0x679DD514582F9FCF,
                0x0F6D2B697BD44DA8,
                0x77E36F7304C48942,
                0x3F9D85A86A1D36C8,
                0x1112E6AD91D692A1,
            ],
            size: 0,
        }
    }
}

impl Sha64<256> {
    pub const fn new() -> Self {
        Self {
            h: [
                0x22312194FC2BF72C,
                0x9F555FA3C84C64C2,
                0x2393B86B6F53B151,
                0x963877195940EABD,
                0x96283EE2A88EFFE3,
                0xBE5E1E2553863992,
                0x2B0199FC2C85B8AA,
                0x0EB72DDC81C52CA2,
            ],
            size: 0,
        }
    }
}

impl<const BITS: usize> Sha64<BITS> {
    pub const fn with_iv(h: [u64; 8]) -> Self {
        Self { h, size: 0 }
    }

    pub fn generate_iv() -> Self {
        assert!(BITS < 512);
        assert_ne!(BITS, 384);
        let mut modified = {
            let mut sha512 = Sha64::<512>::new();
            for h in &mut sha512.h {
                *h ^= 0xa5a5a5a5a5a5a5a5;
            }
            sha512
        };
        let msg = format!("SHA-512/{}", BITS);
        let mut out = [0; 64];
        modified.do_final(msg.as_bytes(), &mut out);
        let mut h = [0; 8];
        for (i, v) in h.iter_mut().zip(bytemuck::cast_slice::<u8, [u8; 8]>(&out)) {
            *i = u64::from_be_bytes(*v);
        }

        Self::with_iv(h)
    }
}

impl Default for Sha64<384> {
    fn default() -> Self {
        Self::new()
    }
}

impl Default for Sha64<512> {
    fn default() -> Self {
        Self::new()
    }
}

impl Default for Sha64<224> {
    fn default() -> Self {
        Self::new()
    }
}

impl Default for Sha64<256> {
    fn default() -> Self {
        Self::new()
    }
}

impl<const BITS: usize> Digest for Sha64<BITS> {
    const BLOCK_SIZE: usize = 128;
    const OUTPUT_SIZE: usize = BITS / 8;

    fn init(&mut self) {
        self.size = 0;
        self.h = match BITS {
            224 => [
                0x8C3D37C819544DA2,
                0x73E1996689DCD4D6,
                0x1DFAB7AE32FF9C82,
                0x679DD514582F9FCF,
                0x0F6D2B697BD44DA8,
                0x77E36F7304C48942,
                0x3F9D85A86A1D36C8,
                0x1112E6AD91D692A1,
            ],
            256 => [
                0x22312194FC2BF72C,
                0x9F555FA3C84C64C2,
                0x2393B86B6F53B151,
                0x963877195940EABD,
                0x96283EE2A88EFFE3,
                0xBE5E1E2553863992,
                0x2B0199FC2C85B8AA,
                0x0EB72DDC81C52CA2,
            ],
            384 => [
                0xcbbb9d5dc1059ed8,
                0x629a292a367cd507,
                0x9159015a3070dd17,
                0x152fecd8f70e5939,
                0x67332667ffc00b31,
                0x8eb44a8768581511,
                0xdb0c2e0d64f98fa7,
                0x47b5481dbefa4fa4,
            ],
            512 => [
                0x6a09e667f3bcc908,
                0xbb67ae8584caa73b,
                0x3c6ef372fe94f82b,
                0xa54ff53a5f1d36f1,
                0x510e527fade682d1,
                0x9b05688c2b3e6c1f,
                0x1f83d9abfb41bd6b,
                0x5be0cd19137e2179,
            ],
            _ => Self::generate_iv().h,
        }
    }

    fn update(&mut self, block: &[u8]) {
        self.size += BITS as u128;
        do_sha64_block(block, &mut self.h);
    }

    fn do_final(&mut self, mut lblock: &[u8], out: &mut [u8]) {
        assert!(lblock.len() <= Self::BLOCK_SIZE);
        if lblock.len() == Self::BLOCK_SIZE {
            self.update(lblock);
            lblock = &[];
        }
        let len = lblock.len();
        let mut bytes = [0u8; 128];
        bytes[..len].copy_from_slice(lblock);
        self.size += (lblock.len() as u128) * 8;
        let ml = self.size;
        bytes[len] = 0x80;
        if (Self::BLOCK_SIZE - len) < 16 {
            self.update(&bytes);
            bytes = [0u8; 128];
        }
        let len = bytes.len() - 16;
        bytes[len..].copy_from_slice(&ml.to_be_bytes());
        self.update(&bytes);
        bytes.zeroize();
        let mut output = [0u8; 64];
        for (k, v) in bytemuck::cast_slice_mut::<u8, [u8; 8]>(&mut output)
            .iter_mut()
            .zip(&self.h)
        {
            *k = v.to_be_bytes();
        }

        out.copy_from_slice(&output[..(BITS / 8)])
    }
}

pub type Sha384 = Sha64<384>;
pub type Sha512 = Sha64<512>;
pub type Sha512_224 = Sha64<224>;
pub type Sha512_256 = Sha64<256>;
