...
impl<const N: usize> Zeroize for Aes<{ N }> {
    fn zeroize(&mut self) {
        self.rkeys.zeroize()
    }
}

impl<const N: usize> Drop for Aes<{ N }> {
    fn drop(&mut self) {
        self.zeroize()
    }
}
...
static MIX_COLUMNS_BY11: [u8; 256] = [
...
static MIX_COLUMNS_BY13: [u8; 256] = [
...
static MIX_COLUMNS_BY14: [u8; 256] = [
...
fn aes_generate_keys(key: &[u8], rkeys: &mut [[u8; 16]]) {
    let key = bytemuck::cast_slice::<u8, [u8; 4]>(key);
    let rkeys = bytemuck::cast_slice_mut::<[u8; 16], [u8; 4]>(rkeys);
    let kwords = key.len();
    rkeys[..kwords].copy_from_slice(key);
    for i in kwords..rkeys.len() {
        let mut word = Zeroizing::new([0u8; 4]);
        word.copy_from_slice(&rkeys[i - 1]);
        match i % kwords {
            0 => {
                word.rotate_left(1);
                for r in &mut *word {
                ...
                *bytemuck::cast_mut::<[u8; 4], u32>(&mut *word) ^= RCON[i / kwords];
            }
            4 if kwords > 6 => {
                for r in &mut *word {
                    *r = SBOX[*r as usize];
        ...
        *bytemuck::cast_mut::<[u8; 4], u32>(&mut *word) ^=
            *bytemuck::cast_ref::<[u8; 4], u32>(&rkeys[i - kwords]);
...
fn aes_do_enc_round(block: &mut [u8], rkey: &[u8; 16]) {
    ...
    for i in 0..4 {
        block[i].rotate_left(i)
    }

    aes_mix_columns(block);
    for i in 0..4 {
        *bytemuck::cast_mut::<[u8; 4], u32>(&mut block[i]) ^=
            bytemuck::cast_slice::<u8, u32>(rkey)[i];
    }
}
...
impl SymmetricCipher for Aes<128> {
    const BLOCK_SIZE: usize = 16;

    const KEY_SIZE: usize = 16;

    fn init(&mut self, key: &[u8], op: super::Operation) {
        self.zeroize();
        self.operation = Some(op);
        aes_generate_keys(key, &mut self.rkeys[..11]);
    }

    fn update(&mut self, block: &[u8], out: &mut [u8]) {
        out.copy_from_slice(block);
        match self.operation {
            Some(Operation::Encrypt) => aes_encrypt(out, &self.rkeys[..11]),
            Some(Operation::Decrypt) => aes_decrypt(out, &self.rkeys[..11]),
...
impl SymmetricCipher for Aes<192> {
    const BLOCK_SIZE: usize = 16;
    ...
    fn init(&mut self, key: &[u8], op: super::Operation) {
        self.zeroize();
        self.operation = Some(op);
        aes_generate_keys(key, &mut self.rkeys[..13]);
    }

    fn update(&mut self, block: &[u8], out: &mut [u8]) {
        out.copy_from_slice(block);
        match self.operation {
            Some(Operation::Encrypt) => aes_encrypt(out, &self.rkeys[..13]),
            Some(Operation::Decrypt) => aes_decrypt(out, &self.rkeys[..13]),
            _ => panic!("Uninitialized"),
        }
    }
...
impl<const N: usize> Default for Aes<N> {
    fn default() -> Self {
        Self::const_new()
    }
}

impl SymmetricCipher for Aes<256> {
    const BLOCK_SIZE: usize = 16;
    ...
    fn init(&mut self, key: &[u8], op: super::Operation) {
        self.zeroize();
        self.operation = Some(op);
        aes_generate_keys(key, &mut self.rkeys[..15]);
    }

    fn update(&mut self, block: &[u8], out: &mut [u8]) {
        out.copy_from_slice(block);
        match self.operation {
            Some(Operation::Encrypt) => aes_encrypt(out, &self.rkeys[..15]),
            Some(Operation::Decrypt) => aes_decrypt(out, &self.rkeys[..15]),
            _ => panic!("Uninitialized"),
        }
    }
